...
cardapio.metodos = {
    ...
    carregarCarrinho: () => {
        ...
        // Atualiza preview de observação ao abrir o carrinho (se houver)
        let obsPreview = $('#txtObservacao').val() ? $('#txtObservacao').val().trim() : ''
        if(obsPreview.length > 0) {
            $("#observacoesPreview").text('Observação: ' + obsPreview)
        } else {
            $("#observacoesPreview").text('')
        }
    ...
    carregarResumo: () => {
        ...
        // Atualiza preview da observação (se existir)
        let obs = $('#txtObservacao').val() || ''
        if(obs.trim().length > 0) {
            $("#observacoesPreview").text('Observação: ' + obs.trim())
        } else {
            $("#observacoesPreview").text('')
        }
        // Mostra a observação no resumo (se existir)
        if(obs.trim().length > 0) {
            $("#resumoObservacao").text(obs.trim())
            $("#resumoObservacaoContainer").show()
        } else {
            $("#resumoObservacao").text('')
            $("#resumoObservacaoContainer").hide()
        }
    ...
    finalizarPedido: () => {
        ...
        if (MEU_CARRINHO.length > 0 && MEU_ENDERECO != null) {
            ...
            // Anexa observação do cliente, se houver
            let observacao = $('#txtObservacao').val() ? $('#txtObservacao').val().trim() : ''

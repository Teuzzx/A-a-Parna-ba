...
var CELULAR_ESTABELECIMENTO = "5551999355535"
...
cardapio.metodos = {
    ...
    adicionarAoCarrinho: (id) => {
        ...
        if (qntdAtual > 0) {
            ...
            if (item.length > 0) {
                // Como agora cada item pode ter ingredientes diferentes, vamos adicionar como um novo item se for açaí
                // Ou se quiser manter a lógica de agrupar, precisaria comparar os ingredientes.
                // O prompt diz: "Adicionar apenas: Açaí - Tamanho G. Ainda sem ingredientes selecionados."
    ...
    carregarCarrinho: () => {
        ...
        if (MEU_CARRINHO.length > 0) {
            ...
            $.each(MEU_CARRINHO, (i, e) => {
                ...
                // Adiciona ingredientes se houver
                let desc = "";
                if (e.coberturas.length > 0) desc += "Coberturas: " + e.coberturas.join(', ') + "<br>";
                if (e.complementos.length > 0) desc += "Ingredientes: " + e.complementos.join(', ');
    ...
    abrirModalIngredientes: (idCarrinho) => {
        let item = MEU_CARRINHO.find(e => e.idCarrinho == idCarrinho);
        if (item) {
            ...
            // Carrega Coberturas
            $.each(INGREDIENTES.coberturas, (i, e) => {
                let checked = item.coberturas.includes(e.name) ? 'checked' : '';
                $("#listaCoberturas").append(`
                    <div class="col-12 col-lg-6">
                        <div class="form-check">
                            <input class="form-check-input check-cobertura" type="checkbox" value="${e.name}" id="cb-${e.id}" ${checked}>
                            <label class="form-check-label" for="cb-${e.id}">${e.name}</label>
                        </div>
                    </div>
                `);
            });
            ...
            $.each(INGREDIENTES.complementos, (i, e) => {
                let checked = item.complementos.includes(e.name) ? 'checked' : '';
                $("#listaComplementos").append(`
                    <div class="col-12 col-lg-6">
                        <div class="form-check">
                            <input class="form-check-input check-complemento" type="checkbox" value="${e.name}" id="cp-${e.id}" ${checked}>
                            <label class="form-check-label" for="cp-${e.id}">${e.name}</label>
                        </div>
                    </div>
                `);
            });

            $("#btnConfirmarIngredientes").off('click').on('click', () => {
                let coberturas = [];
                $(".check-cobertura:checked").each(function() { coberturas.push($(this).val()); });
                
                let complementos = [];
                $(".check-complemento:checked").each(function() { complementos.push($(this).val()); });

                if (coberturas.length > 2) {
                    cardapio.metodos.mensagem("Você pode escolher até 2 coberturas.");
                    return;
                }
                if (complementos.length > 7) {
                    cardapio.metodos.mensagem("Você pode escolher até 7 ingredientes.");
                    return;
                }
    ...
    carregarResumo: () => {
        ...
        if (MEU_ENDERECO.forma == 'Entrega (Delivery)') {
            ...
            $("#cidadeEndereco").html('Endereço: Rua Exemplo, 123 - Centro')
        }
    ...
    finalizarPedido: () => {
        if (MEU_CARRINHO.length > 0 && MEU_ENDERECO != null) {
            var texto = '📦 *NOVO PEDIDO DE AÇAÍ*\n\n';

            $.each(MEU_CARRINHO, (i, e) => {
                texto += `🥣 *Produto:* Açaí\n`;
                texto += `📏 *Tamanho:* ${e.dsc}\n`;
                
                if (e.coberturas.length > 0) {
                    texto += `🍫 *Coberturas escolhidas:* ${e.coberturas.join(', ')}\n`;
                }
                if (e.complementos.length > 0) {
                    texto += `🥄 *Ingredientes:* ${e.complementos.join(', ')}\n`;
                }
                texto += `\n`;
            });
            ...
            $.each(MEU_CARRINHO, (i, e) => {
                texto += `Açaí ${e.dsc} — R$ ${e.price.toFixed(2).replace('.', ',')}\n`;
            });
            
            if (VALOR_ENTREGA > 0) {
                texto += `Frete — R$ ${VALOR_ENTREGA.toFixed(2).replace('.', ',')}\n`;
            } else {
                texto += `Frete — Grátis\n`;
            }
            ...
            if (MEU_ENDERECO.forma == 'Entrega (Delivery)') {
                texto += `📍 *Endereço do cliente:* ${MEU_ENDERECO.endereco}, ${MEU_ENDERECO.numero}, ${MEU_ENDERECO.bairro} - ${MEU_ENDERECO.cidade}/${MEU_ENDERECO.uf}\n`;
            }
...
cardapio.templates = {
    ...
    <div class="col-12 item-carrinho">
        ...
        <div class="dados-produto">
            ...
            <a class="btn btn-yellow btn-sm mt-2" onclick="cardapio.metodos.abrirModalIngredientes('\${id}')">Escolher ingredientes</a>
        </div>
        <div class="add-carrinho">
